...
MEPSTEMP.namespace("UI.ProbeType.Search");
MEPSTEMP.UI.ProbeType.Search = (function () {
    "use strict";
    var _isInitialized = false;
   
    var $ddlSearch, $txtSearch, $ddlSystem, $ddlOrganization, $DivProbeTypeGrid, $btnProbeTypeSearch;
    ...
    function initialiseControls() {
        ...
        function CacheEl() {
            ...
            $DivProbeTypeGrid = $('#DivProbeTypeGrid');
            $btnProbeTypeSearch = $('#btnProbeTypeSearch');
        }
        //Event binding done in here
        function BindEvents() {
            MEPSTEMP.Global.ApplyMenuSelection('ProbeType');
            BindGridEvents();
            ...
            //Search button event
            $btnProbeTypeSearch.click(function () {
                SearchProbeType();
            });
           
        }
        function BindGridEvents() {
            MEPSTEMP.Global.BindDeletionModal(MEPSTEMP.UI.ProbeType.Search.resetGrid);
            $('.btnProbeTypeEdit').each(function () {
                $(this).click(function () {
                    $('#hdnProbeTypeId').val($(this).data('probetypeid'));
                    $('#ProbeTypeSearch').submit();
                });
            });
            ...
            MEPSTEMP.Global.LoadAJAXPagination(
               ...
               //pagination window end page number - default is 3 -carefull : if this is set more than total pages then unwanted behaviour may appear
                ...
                'ProbeTypeSearch/FilterProbeTypeSearch',
                 // success call back
                function (result) {
                    $DivProbeTypeGrid.html(result);
        ...
        function SearchProbeType() {
            var data = {};
            data.SearchField = $ddlSearch.val();
            data.SearchText = $txtSearch.val();
            var url = 'ProbeTypeSearch/FilterProbeTypeSearch';
